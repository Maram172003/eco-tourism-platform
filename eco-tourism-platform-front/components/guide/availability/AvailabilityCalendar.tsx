...
export default function AvailabilityCalendar({ token, readOnly = false }: Props) {
  const {
    slots, loading,
  ...
  const handleSubmit = useCallback(async (slot: Omit<AvailabilitySlot, "id">) => {
    ...
    // Keep recurDays so guide can add same pattern again
  }, [createSlot]);
  ...
  // ── Delete ────────────────────────────────────────────────────────────────────
  const handleDelete = useCallback((id: string) => deleteSlot(id), [deleteSlot]);
  ...
  const handleEditSave = useCallback(async (id: string, updated: Omit<AvailabilitySlot, "id">) => {
    ...
    // Strip backend-only fields (guide_id, created_at…) before sending
  ...
  }, [slots, deleteSlot, createSlotForced]);
